...
interface CanvasSingleSelectionForegroundProps {
  isDraggable: boolean;
  isResizable: boolean;
  isRotatable: boolean;
  nodeId: string;
...
export const CanvasSingleSelectionForeground = ({
  isDraggable,
  isResizable,
  isRotatable,
  nodeId,
  selectionGhost,
}: CanvasSingleSelectionForegroundProps) => {
    ...
    <ContextMenu>
      <ContextMenuTrigger
        render={
          ...
          <div
            className={`canvas-moveable canvas-single-selection moveable-control-box absolute ${cursorClassName}`}
            onDoubleClick={(event) => {
              event.preventDefault();
              event.stopPropagation();
              openCanvasNodeEditingMode(editor, nodeId, {
      ...
      >
        <CanvasSelectionGhost ghost={selectionGhost} />
