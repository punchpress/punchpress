...
import { getBrushColorRgb } from "./brush-settings";
import {
...
export type RasterStrokeRuntimeSession = {
  readonly delegate?: RasterStrokeRuntimeSession | null;
...
type RasterStrokeHandoff = {
  status?: "failed" | "presented";
};

type ManagedRasterStrokeSession = RasterStrokeRuntimeSession & {
  readonly delegate?: ManagedRasterStrokeSession | null;
  readonly requiresFiniteInputClipping?: boolean;
  readonly workingSurfaceId?: string | null;
  acknowledgeWorkingPresentation?: (
    acknowledgement: RasterPresentationAcknowledgement
  ) => boolean;
  completeCurrent?: () => Promise<unknown> | unknown;
  failWorkingPresentation?: (failure: RasterPresentationFailure) => boolean;
  getCommitReady?: () => Promise<unknown>;
  getFollowupReady?: () => Promise<RasterStrokeHandoff | void>;
  getHandoffReady?: () => Promise<RasterStrokeHandoff | void>;
  getWorkingGroup?: () => RasterWorkingGroup | null;
  getWorkingNodeId?: () => string | null;
  hasPendingWorkingSurface?: () => boolean;
  invalidateWorkingPresentation?: () => void;
  resolveHistoryForInvalidation?: () => void;
...
class BrushStrokeSession {
  ...
  commit() {
    ...
    const committedCanvas: any = measurePerf("brush.commit.prepareCanvas", () =>
...
class CommitQueuedBrushStrokeSession {
  ...
  constructor({ operation, point, settings, targetState, tool, waitFor }) {
    this.canceled = false;
...
export class RasterStrokeRuntime {
  readonly editor: any;
  activeSession: ManagedRasterStrokeSession | null;
  pendingWorkingSurfaces: { session: ManagedRasterStrokeSession }[];
  ...
  getWorkingGroups(): RasterWorkingGroup[] {
  ...
  getWorkingPresentationForNode(
    nodeId: string
  ): RasterWorkingPresentation | null {
  ...
  retireWorkingPresentations(groupIds: ReadonlySet<string>): void {
  ...
  invalidateWorkingPresentations(nodeId: string | null = null): void {
    for (const session of this.getSessions()) {
  ...
  invalidateMissingWorkingPresentations(): void {
    for (const session of this.getSessions()) {
  ...
  getSessions(): ManagedRasterStrokeSession[] {
    return [
  ...
  beginStroke({
    ...
    point,
    settings,
  }: BeginRasterStrokeInput): RasterStrokeRuntimeSession | null {
    return measurePerf("brush.stroke.begin", () => {
      ...
      const precedingTargetSession = targetNodeId
        ? [...this.pendingWorkingSurfaces]
            .reverse()
            .find(
              (entry) =>
                entry.session.getWorkingNodeId?.() === targetNodeId &&
                entry.session.hasPendingWorkingSurface?.()
            )?.session
        : null;
      const session = makeRasterSessionIdempotent(
        precedingTargetSession
          ? new CommitQueuedBrushStrokeSession({
              ...
              tool: this,
              waitFor:
                precedingTargetSession.getFollowupReady?.() ||
                precedingTargetSession.getHandoffReady?.(),
            })
  ...
  beginResolvedStroke({
    ...
    point,
    settings,
  }: BeginRasterStrokeInput & { node: any }): ManagedRasterStrokeSession | null {
  ...
  beginResidentStroke({
    ...
    point,
    settings,
  }: BeginRasterStrokeInput & {
    node: any;
  }): ManagedRasterStrokeSession | null {
    if (
      !(
    ...
    let state: "active" | "cancelled" | "completed" = "active";
    let completion: Promise<any> = Promise.resolve();
    const finish = () => {
    ...
    const session = {
      ...
      complete: ({ point: endPoint }) => {
        ...
        completion = Promise.resolve(commit);
        finish();
        return completion;
      },
      getWorkingGroup: () => null,
  ...
  clearPendingPreview(session: ManagedRasterStrokeSession): void {
  ...
  clearActiveSession(session: ManagedRasterStrokeSession): void {
